...
export default async function AutonomyPage() {
  ...
  if (supabase) {
    const [ev, vid, snap, q, r] = await Promise.all([
      ...
      scopeQuery(supabase.from("content_queue").select("*"), scope).order("added_at", { ascending: false }).limit(200),
      scopeQuery(supabase.from("pipeline_runs").select("*"), scope).order("updated_at", { ascending: false }).limit(200),
    ]);
