...
* the clip goes to the PRIVATE Storage bucket ``voice-previews`` as
  ``<voice id>.mp3`` (migration 0026). Signed-in users get a short-lived
...
class Previewer:
    ...
    def exists(self, voice_id: str) -> bool:
        ...
        if resp.status_code != 200:
            raise PreviewError(f"{voice_id}: listing the bucket failed (HTTP {resp.status_code}); is 0026 applied?")
